...
export default function outputfunc(args, line, trace, compiled) {
	try {
		...
		return [{commands: `mov eax,4\nmov ebx,1\nmov ecx,${args[0]}\nmov edx,${args[0]}len\nint 0x80\n`, type: "text", os: ['win', 'linux']}, {commands: `mov eax,0x20000004\nmov ebx,0x20000001\nmov ecx,${args[0]}\nmov edx,${args[0]}len\nint 0x80\n`, type: "text", os: ['mac']}, {label: args[0], commands: `db "${args[0]}: ${args[1]}",10,0`, type: "label", os: ['mac', 'linux', 'win']}, {label: args[0]+"len", commands: `equ $-${args[0]}`, type: "label", os: ['mac', 'linux', 'win']}]
